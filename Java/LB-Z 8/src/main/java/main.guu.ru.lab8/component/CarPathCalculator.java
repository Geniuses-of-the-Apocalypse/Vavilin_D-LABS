...
@Service
@Log4j2
public class CarPathCalculator {
    @Autowired
    ...
    public void onRouteChange(RouteChangeEvent event) {
        ...
        currentPath = graphManager.getPath(start, end, TransportType.CAR);
        currentDistance = calculateDistance();

        log.info("Автомобиль: путь пересчитан");
    }
    ...
    private double calculateDistance() {
        ...
        for (int i = 0; i < currentPath.size() - 1; i++) {
            ...
            for (var edge : graphManager.getEdgesFrom(from)) {
                if (edge.getTo().equals(to) && edge.isAvailableForCar()) {
    ...
    public String toString() {
        return "Автомобиль{путь=" + currentPath + ", расстояние=" + currentDistance + '}';
    }
}